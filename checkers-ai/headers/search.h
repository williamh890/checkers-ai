...
EvaluationType search(const ActionType& action, int depth,
...
class SearchHelper {
 public:
  ...
  EvaluationType recurse(const CheckersGame::MovePackage& move, int depth,
                         EvaluationType alpha, EvaluationType beta);
  EvaluationType recurse(const CheckersGame::JumpPackage& jump, int depth,
                         EvaluationType alpha, EvaluationType beta);
  ...
  EvaluationType recurseMultiJumpCase(
      const std::vector<CheckersGame::JumpPackage>& multiJumps, int depth,
      EvaluationType alpha, EvaluationType beta);

  EvaluationType recursiveCase(int depth, EvaluationType alpha,
                               EvaluationType beta);
...
template <typename ActionType>
ai::EvaluationType ai::search(const ActionType& action, int depth,
                              char maximizingPlayer, CheckersGame& game) {
