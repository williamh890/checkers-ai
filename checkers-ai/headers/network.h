...
class Network {
 public:
  ...
  Settings::NetworkWeightType evaluateBoard(
      const std::vector<char> & /*inputBoard*/, bool testing = false,
      int red_factor = 1);
 ...
 private:
  ...
  void evolveSigmas();
  void inline evolveSigmaAt(size_t i, size_t ii,
                            Settings::NetworkWeightType tau);
  void evolveWeights();
  Settings::NetworkWeightType inline evolveWeightAt(size_t i, size_t ii);

  template <typename RandomNumberType>
  std::vector<RandomNumberType> getRandomNumbersOfLength(
      const unsigned int length,
      std::uniform_real_distribution<RandomNumberType> &distribution);

  void calculateNode(unsigned int /*x*/, unsigned int /*y*/);
  inline Settings::NetworkWeightType activationFunction(
      Settings::NetworkWeightType x);
...
void setupNetworks(const std::vector<unsigned int> &dimensions,
                   int numberOfNetworks = NETWORKPOPSIZE);
Settings::NetworkWeightType getGaussianNumber(
    std::mt19937 & /*randomNumGenerator*/);
