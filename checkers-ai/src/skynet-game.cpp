...
//      ./skynet-game.out <game-name> <player-color> <network-id>
//      <search-depth-limit>
...
struct GameInfo {
  ...
  std::string board;
  GameInfo() = default;
  GameInfo(std::string turn, std::string board)
      : turn(std::move(turn)), board(std::move(board)) {}
};

void sendTurn(const std::string &gameName, const std::string &board) {
  std::string sendCmd = "python3 pyintheskynet/skynet.py play_game --game " +
                        gameName + " --board " + board;
  sendCmd += server;
...
GameInfo getGameInfo(const std::string &game) {
  ...
  std::string listCmd = "python3 pyintheskynet/skynet.py game_info --game " +
                        game + " > " + cmdOutfile;
  listCmd += server;
...
std::string getNextBoardState(const std::string &gameName,
...
void skynetPlay(const std::string &gameName, const char playerColor,
...
int main(int /*argc*/, char **argv) {
  ...
  ai::CheckersGame game;
  if (network_id < 0) {
    game = (playerColor == 'r') ? ai::getCheckersGame() : ai::getCheckersGame();
  } else {
    game = (playerColor == 'r') ? ai::getNetworkedCheckersGame(network_id, 0)
