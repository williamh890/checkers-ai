...
int main(int argc, char *argv[]) {
  ...
  if (*argv[1] == SETUP_NETWORKS) {
    ...
    create_directories(path)
        ? cout << "Your directory was created" << endl
    ...
    cout << "There are: " << folderPaths.size() << " networks in development"
         << endl;
    ...
    // sort(myNetworks.begin(), myNetworks.end(), [](Network first, Network
    ...
    for (auto i : networkPathsContainer) {
      ...
      // endl; for (auto a : networks_in_one_folder) {
    ...
    unsigned int numNetworksToSteal =
        (NETWORKPOPSIZE - leftovers) / networkPathsContainer.size();
